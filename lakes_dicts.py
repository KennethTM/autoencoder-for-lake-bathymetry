...
def main():
  ...
  lake_max_depth_thresh = 1

  #Read lake summary
  summary = pd.read_csv("data/lakes_summary.csv")
  summary_sub = summary.copy()
  summary_sub = summary_sub[(summary["area"] > lake_size_thresh) & (summary["max_depth"] > lake_max_depth_thresh)]

  #Create column for partioning of lakes
  #Split into train/val/test sets of 60/20/20 %
  train_len = int(summary_sub.shape[0]*0.6)
  valid_len = int(summary_sub.shape[0]*0.2)+1
  test_len = int(summary_sub.shape[0]*0.2)+1
  ...
  #Loop through lakes and prepare arrays
  ...
  summary_sub.to_csv("data/lakes_summary_partition.csv", index=False)
